...
Coaching reports are generated by `POST /api/v1/reports` (PDF/CSV/JSON), uploaded to the `artifacts` R2 bucket, and downloaded via presigned URLs from `GET /api/v1/reports/{id}/download`. PDF rendering uses `reportlab` (pure-Python — no system libraries to install). See [`reports/templates/coaching_summary.md`](reports/templates/coaching_summary.md) for the template contract.
