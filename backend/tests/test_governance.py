    ...
    async def _db() -> AsyncGenerator[Any, None]:
        ...
        async def _execute(_stmt: Any) -> Any:
            ...
            scalars.all.return_value = (
                list_rows if list_rows is not None else (audit_rows or [])
            )
...
def test_policy_default_deny_for_unknown_pair() -> None:
    ...
    assert (
        policy_allows(UserRole.admin, Resource.PLAYER_PROFILE, Action.TRIGGER) is False
    )


def test_policy_allows_expected_roles() -> None:
    assert policy_allows(
        UserRole.sportsperformance, Resource.HEALTH_WORKLOAD, Action.READ
    )
    assert not policy_allows(
        UserRole.coach, Resource.HEALTH_WORKLOAD, Action.READ
    )
    assert policy_allows(
        UserRole.admin, Resource.PLAYER_VISIBILITY, Action.APPROVE
    )
    assert not policy_allows(
        UserRole.coach, Resource.PLAYER_VISIBILITY, Action.APPROVE
    )
...
def test_resolve_visibility_mode_admin_can_request_any() -> None:
    ...
    assert (
        resolve_visibility_mode(user, VisibilityMode.RECRUITING)
        == VisibilityMode.RECRUITING
    )
