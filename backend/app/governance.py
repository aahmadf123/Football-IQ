...
POLICY: dict[tuple[Resource, Action], frozenset[UserRole]] = {
    ...
    (Resource.PLAYER_VISIBILITY, Action.APPROVE): frozenset(
    ...
    (Resource.HEAVY_WORKLOAD, Action.TRIGGER): frozenset(
        {UserRole.admin, UserRole.analyst}
    ),
}
        ...
        if not policy_allows(user.role, resource, action):
            ...
            raise HTTPException(
                ...
                detail={
                    ...
                    "action": action.value,
                    "message": (
                        f"Role '{user.role}' is not permitted to {action.value} "
                        f"{resource.value}"
                    ),
                },
            )
...
_SENSITIVE_PLAYER_FIELDS: frozenset[str] = frozenset(
    {"metadata", "user_id"}
)
...
_ROLE_CAN_REQUEST_MODE: dict[UserRole, frozenset[VisibilityMode]] = {
    ...
    UserRole.coach: frozenset(
        {VisibilityMode.STAFF, VisibilityMode.RECRUITING}
    ),
...
def resolve_visibility_mode(
    user: User, requested: VisibilityMode | None
) -> VisibilityMode:
    ...
    raise HTTPException(
        ...
        detail={
            ...
            "requested_mode": requested.value,
            "message": (
                f"Role '{user.role}' may not request visibility mode "
                f"'{requested.value}'."
            ),
        },
    )
